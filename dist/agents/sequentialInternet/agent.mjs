...
export async function runSequentialInternetAgent(request) {
    const { prompt, n_queries = 3, max_recursion = 10, service, model } = request;
    // Recursive helper function
    async function sequentialSearch(currentPrompt, recursionCount) {
...
User prompt: "${currentPrompt}"
    `;
        ...
        const serviceName = service?.name || "tavily";
        const serviceAk = service?.ak || model.ak;
...
Please produce a final, comprehensive answer that integrates these search results with clear citations.
    `;
        ...
        const candidateAnswer = await promptLlm(finalPrompt, model);
