...
// --------------------------------------------------------------------
export async function clickCoordinates(page, x, y) {
...
async function getActiveElementDescriptor(page) {
...
// --------------------------------------------------------------------
export async function runBrowserUseAgent(request) {
    const { prompt, model } = request;
    const maxSteps = request.maxSteps || 25;
    let step = 0;
    let finished = false;
    const actions = [];
    let lastClickedSelector = "";
    ...
    while (!finished && step < maxSteps) {
        ...
        contextLines.push({ type: "text", text: "Based on the above, decide the next action. Your response must be valid JSON following this schema:" }, { type: "text", text: `{
  ...
  "parameters": {
    ...
    // For "finish": {}
        ...
        const conversation = [
            ...
            { role: "user", content: [{ type: "text", text: `User prompt: "${prompt}". Please specify the next browser action.` }] },
        ];
