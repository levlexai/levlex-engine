...
export async function promptLlmJsonOutput(prompt, model) {
    ...
    const completion = await openai.chat.completions.create({
        ...
        messages: [
            {
                role: "system",
                content: "You are a helpful assistant. Please output your answer strictly as valid JSON without any additional text, commentary, or markdown formatting.",
            },
            { role: "user", content: prompt },
        ],
...
export async function promptLlmWithSchemaAndConversation(model, messages, outputSchema) {
    const openai = new OpenAI({
        apiKey: model.ak,
        baseURL: model.base_url,
    });
