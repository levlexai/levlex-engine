...
router.post("/startCall", async (req, res) => {
    try {
        ...
        const client = new Twilio(phoneAgentConfig.twilioAccountSid, phoneAgentConfig.twilioAuthToken);
        // Create the call
...
router.post("/call", async (req, res) => {
    ...
    if (state.lastQuestion && userInput) {
        try {
            const updated = await updateJsonUsingLLM(state.partialJson, userInput, promptForJson, phoneAgentConfig.llmModel);
            state.partialJson = updated.newJson;
            state.done = updated.done;
            if (state.done) {
                return respondWithTwiML(res, hangupResponse("Thank you! Your data is complete. Goodbye."));
            }
        }
        catch (err) {
            console.error("Error updating JSON:", err);
            return respondWithTwiML(res, hangupResponse("An error occurred. Goodbye."));
        }
    }
    ...
    try {
        ...
        return respondWithTwiML(res, hangupResponse("An error occurred generating next question. Goodbye."));
    }
    ...
    try {
        ...
        return respondWithTwiML(res, gatherTwiML("Failed to generate audio. Please enter digits."));
    }
